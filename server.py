...
import os
import time
import json  # ✅ Import JSON for safe Redis caching
import datetime
import io
...
app = Flask(__name__)
CORS(app)  # Allow all origins (Not recommended for production)
...
def upload_image():
    try:
        ...
        if image:
            ...
            # ✅ Store embeddings correctly in MongoDB
            db.image_metadata.insert_one({
                "image_id": str(image_id),
                "event": event,
                "date": date,
                "face_embeddings": [list(e) for e in embeddings],  # ✅ Convert NumPy to list
                "uploaded_at": datetime.datetime.utcnow()
            })
...
def search_image():
    try:
        ...
        if image:
            ...
            if cached_results:
                print("✅ Using Cached Search Results")
                results = json.loads(cached_results.decode())  # ✅ Safe JSON conversion
            else:
                results = match_faces(image_path, event, date)
                ...
                for match in results.get("matches", []):
                    image_id = match["image_id"]
                    match["image_url"] = f"http://127.0.0.1:5000/get_image/{image_id}" 
                    match["download_url"] = f"http://127.0.0.1:5000/download/{image_id}"  

                redis_client.setex(cache_key, 3600, json.dumps(results))  # ✅ Cache as JSON
...
def download_image(image_id):
    try:
        ...
        return send_file(
            ...
            mimetype=image_data.content_type,  # ✅ Use correct MIME type
            as_attachment=True,
            download_name=f"{image_id}.jpg"
        )
    except Exception as e:
...
def get_image(image_id):
    try:
        ...
        response = Response(image_data.read(), mimetype=image_data.content_type)  # ✅ Correct MIME type
        response.headers["Content-Disposition"] = f'inline; filename="{image_data.filename}"'
